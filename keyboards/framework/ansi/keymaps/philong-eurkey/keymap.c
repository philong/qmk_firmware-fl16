...
// EurKEY host variant of the philong keymap (QWERTY-based EurKEY layout)
//
...
bool caps_word_press_user(uint16_t keycode) {
    ...
    bool get_chordal_hold(
        ...
        // LGui+L screen lock protection: force HM_A to tap when L is pressed
        if (tap_hold_keycode == HM_A && get_tap_keycode(other_keycode) == KC_L) {
            return false;
        }
...
// AltGr+letter. Each vowel carries its most-used French accent (E=é, A=à,
// I=î, O=ô, U=ù); the remaining accents sit on neighbors by frequency:
// è (2nd most frequent) on home-row D, then R=ê, W=ë, Q=â, J=û, K=ï, P=ü.
// Letters not listed (ç on C, ñ on N, á on X, …) fall through to EurKEY's
// native AltGr mapping.
